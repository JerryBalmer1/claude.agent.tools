...
Status: done 2026-09-24 - promoted to main as ec807ec
...
- 2026-09-24 - PR 2 on `feature/inspector`: `src/Inspect-Repo.ps1` with five rules, `-Policy`
  ...
  seq 2. Automerge merged #3 as `48ba6d9`.
- 2026-09-24 - PR 3 #4, `develop` -> `main`, merged by claude as `ec807ec`: automerge stood down
  because `main` (the birth commit) had no config. The clauses were re-measured at `48ba6d9` on a
  fresh clone: Full exit 0 with gate 26/0/0/0, every claim red when broken, and the corpus
  regenerated byte-identical (`e2ec679b`). Promotion record: forensic seq 3,
  `promotion-develop-to-main-ec807ec`.
